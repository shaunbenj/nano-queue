...
class Head {
  constructor(down = null, next = null) {
    // Score and previous are fixed to "" and null respectively
    Object.defineProperties(this, {
      isHead: {
        value: true,
        writable: false,
      ...
      score: {
        value: "", // Smallest possible string value
        writable: false,
        configurable: false,
      },
      prev: {
...
// Implementation of a sorted set. Only string scores are allowed
export default class SkipList {
  ...
  insert(score) {
    if (score == null || score == "") {
      throw new Error("Cannot insert null / empty scores into SkipList");
    }
  ...
  getByRange(minScore, maxScore) {
    if (
      minScore.constructor.name != "String" ||
      maxScore.constructor.name != "String"
    ) {
      throw new Error("Scores must be string types");
    }
  ...
  // For new nodes calculate the maximum level it will appear in, 0 indexed.
  #insertLevel() {
    let level = 0;
